## 👋 自己紹介
Webエンジニアとして `要件定義 → 設計 → 実装 → インフラ構築 → 運用/改善` まで一気通貫で担当できるフルスタック寄りのバックエンドエンジニアです。  
※デザインだけできないのは内緒
特に API 開発・設計 / インフラ構築 / DB 設計 / AI・機械学習系 を得意です。
変化を楽しみ、学び続けることを何より大切にしています。

---

## 🛠️ 技術スタック
### **Frontend**
<table>
  <tr>
    <td width="25%" valign="top">
      <h4>
        <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original.svg" width="24" />
        HTML5
      </h4>
      <p>
        <img src="https://img.shields.io/badge/HTML5-E34F26?logo=html5&logoColor=white&style=flat" />
      </p>
    </td>
    <td width="25%" valign="top">
      <h4>
        <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-original.svg" width="24" />
        CSS
      </h4>
      <p>
        <img src="https://img.shields.io/badge/CSS3-1572B6?logo=css3&logoColor=white&style=flat" />
        <img src="https://img.shields.io/badge/Tailwind%20CSS-06B6D4?logo=tailwindcss&logoColor=white&style=flat" />
      </p>
    </td>
    <td width="25%" valign="top">
      <h4>
        <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/typescript/typescript-original.svg" width="30" />
        TypeScript
      </h4>
      <p>
        <img src="https://img.shields.io/badge/React-61DAFB?logo=react&logoColor=black&style=flat" />
      </p>
    </td>
  </tr>
</table>

---

### **Backend**
<table>
  <tr>
    <td width="25%" valign="top">
      <h4>
        <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/php/php-original.svg" width="24" />
        PHP
      </h4>
      <p>
        <img src="https://img.shields.io/badge/Laravel-FF2D20?logo=laravel&logoColor=white&style=flat" />
        <img src="https://img.shields.io/badge/CakePHP-D33C43?logo=cakephp&logoColor=white&style=flat" />
      </p>
    </td>
    <td width="25%" valign="top">
      <h4>
        <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" width="24" />
        Python
      </h4>
      <p>
        <img src="https://img.shields.io/badge/FastAPI-009688?logo=fastapi&logoColor=white&style=flat" />
        <img src="https://img.shields.io/badge/scikit--learn-F7931E?logo=scikit-learn&logoColor=white&style=flat" />
      </p>
    </td>
    <td width="25%" valign="top">
      <h4>
        <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/go/go-original.svg" width="24" />
        Go
      </h4>
      <p>
        <img src="https://img.shields.io/badge/Wails-00ADD8?logo=go&logoColor=white&style=flat" />
      </p>
    </td>
    <td width="25%" valign="top">
      <h4>
        <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/typescript/typescript-original.svg" width="24" />
        TypeScript
      </h4>
      <p>
        <img src="https://img.shields.io/badge/NestJS-E0234E?logo=nestjs&logoColor=white&style=flat" />
      </p>
    </td>
  </tr>
</table>

---

## Infrastructure

<table>
  <tr>
    <td width="25%" valign="top"
        style="border: 1px solid #e1e4e8; border-radius: 12px; padding: 12px;">
      <h4>
        <img src="https://img.shields.io/badge/AWS-232F3E?logo=amazon-aws&logoColor=white&style=flat" width="30" />
        Amazon Web Service
      </h4>
      <p>
        <img src="https://img.shields.io/badge/EC2-FF9900?logo=amazon-aws&logoColor=white&style=flat" />
        <img src="https://img.shields.io/badge/RDS-527FFF?logo=amazon-aws&logoColor=white&style=flat" />
        <img src="https://img.shields.io/badge/S3-F2A30F?logo=amazon-aws&logoColor=white&style=flat" />
        <img src="https://img.shields.io/badge/CloudFront-FF4F00?logo=amazon-aws&logoColor=white&style=flat" />
      </p>
    </td>
    <td width="25%" valign="top"
        style="border: 1px solid #e1e4e8; border-radius: 12px; padding: 12px;">
      <h4>
        <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/linux/linux-original.svg" width="26" />
        Linux
      </h4>
      <p>
        <img src="https://img.shields.io/badge/Amazon%20Linux-232F3E?logo=linux&logoColor=white&style=flat" />
        <img src="https://img.shields.io/badge/Ubuntu-E95420?logo=ubuntu&logoColor=white&style=flat" />
        <img src="https://img.shields.io/badge/CentOS-262577?logo=centos&logoColor=white&style=flat" />
      </p>
    </td>
    <td width="25%" valign="top"
        style="border: 1px solid #e1e4e8; border-radius: 12px; padding: 12px;">
      <h4>
        <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/docker/docker-original.svg" width="26" />
        Docker
      </h4>
      <p>
        <img src="https://img.shields.io/badge/Docker%20Compose-2496ED?logo=docker&logoColor=white&style=flat" />
        <img src="https://img.shields.io/badge/Containerization-2496ED?logo=docker&logoColor=white&style=flat" />
      </p>
    </td>
    <td width="25%" valign="top"
        style="border: 1px solid #e1e4e8; border-radius: 12px; padding: 12px;">
      <h4>
        <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/postgresql/postgresql-original.svg" width="26" />
        Database
      </h4>
      <p>
        <img src="https://img.shields.io/badge/PostgreSQL-4169E1?logo=postgresql&logoColor=white&style=flat" />
        <img src="https://img.shields.io/badge/MySQL-4479A1?logo=mysql&logoColor=white&style=flat" />
        <img src="https://img.shields.io/badge/Redis-DC382D?logo=redis&logoColor=white&style=flat" />
      </p>
    </td>
  </tr>
</table>

---

## 📈 Stats
![](https://raw.githubusercontent.com/shogo-tsunoda/shogo-tsunoda/main/profile-summary-card-output/buefy/0-profile-details.svg)
![](https://raw.githubusercontent.com/shogo-tsunoda/shogo-tsunoda/main/profile-summary-card-output/buefy/3-stats.svg)
![](https://raw.githubusercontent.com/shogo-tsunoda/shogo-tsunoda/main/profile-summary-card-output/buefy/2-most-commit-language.svg)
